...
async function main(argv: string[]): Promise<void> {
  ...
  if (!(await exists(prismaYaml))) {
    return fatal(
      `prisma.yml doesn't exist in "${prismaYaml}". Run \`prisma-upgrade -h\` for more details.`
    )
  }
  ...
  if (prisma2.models.length === 0) {
    return fatal(
      `Your Prisma 2 schema doesn't have any models. Run ${green(
        `\`prisma introspect\``
      )}, then run ${green(`\`prisma-upgrade\``)} again.`
    )
  }
  ...
  if (ops.length) {
    ...
    for (let [type, ops] of segments) {
      ...
      switch (type) {
        case 'SetDefaultOp':
          console.log(
            `  ${bold(`Add missing \`DEFAULT\` constraints to the database`)}`
          )
          console.log(
            `  ${gray(
        ...
        case 'AddUniqueConstraintOp':
          console.log(
            `  ${bold(`Fix 1-1 relations by adding \`UNIQUE\` constraints`)}`
          )
          console.log(
            `  ${gray(
    ...
    console.log(
      ...
      please run the following two commands:
        ...
        )} again to refresh your Prisma 2 schema.
