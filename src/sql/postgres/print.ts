...
function stmt(stmt: sql.Stmt): string {
  switch (stmt.type) {
    ...
    default:
      throw new Error(
        `postgres/print.stmt: unsupported statement "${stmt.type}"`
      )
  }
}
...
function alterTableAction(action: sql.AlterTableAction): string {
  switch (action.type) {
    ...
    default:
      throw new Error(
        `postgres/print.alterTableAction: unsupported action "${action.type}"`
      )
  }
}
...
function alterColumnAction(action: sql.AlterColumnAction): string {
  switch (action.type) {
    ...
    default:
      throw new Error(
        `postgres/print.alterColumnAction: unsupported action "${JSON.stringify(
          action
        )}"`
      )
  }
